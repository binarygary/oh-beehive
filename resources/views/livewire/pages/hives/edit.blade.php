...
new class extends Component
{
    ...
    public function mount(Hive $hive): void
    {
        abort_if($hive->user_id !== auth()->id(), 403);

        $this->hive      = $hive;
        $this->name      = $hive->name;
        $this->location  = $hive->location ?? '';
        $this->acquiredAt = $hive->acquired_at?->format('Y-m-d') ?? '';
        $this->status    = $hive->status->value;
        $this->notes     = $hive->notes ?? '';
    }
...
<x-app-layout>
    <div class="max-w-xl space-y-6">
        ...
        <div>
            ...
            <h1 class="text-2xl font-bold text-base-content">Edit Hive</h1>
            <p class="text-sm text-base-content/50 mt-0.5">{{ $hive->name }}</p>
        </div>
